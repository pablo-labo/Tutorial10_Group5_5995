package defpackage;

import defpackage.bo;
import java.util.List;

/* JADX INFO: loaded from: classes3.dex */
public final class go implements ga<bo.c> {
    public static final go a = new go();
    public static final List<String> b = u63.Z("__typename");

    @Override // defpackage.ga
    public final bo.c a(mb8 mb8Var, lb3 lb3Var) {
        mb8Var.getClass();
        lb3Var.getClass();
        String str = null;
        while (mb8Var.D1(b) == 0) {
            str = (String) ha.a.a(mb8Var, lb3Var);
        }
        mb8Var.k();
        it9 it9VarC = kt9.c(mb8Var, lb3Var);
        if (str != null) {
            return new bo.c(str, it9VarC);
        }
        mh2.q(mb8Var, "__typename");
        throw null;
    }

    @Override // defpackage.ga
    public final void b(hc8 hc8Var, lb3 lb3Var, bo.c cVar) {
        bo.c cVar2 = cVar;
        hc8Var.getClass();
        lb3Var.getClass();
        cVar2.getClass();
        hc8Var.u0("__typename");
        ha.a.b(hc8Var, lb3Var, cVar2.a);
        List<String> list = kt9.a;
        kt9.d(hc8Var, lb3Var, cVar2.b);
    }
}
