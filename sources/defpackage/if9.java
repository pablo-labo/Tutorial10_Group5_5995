...
public final class if9 {
    ...
    public static final void c(float[] fArr, y3a y3aVar) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        y3aVar.a = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        y3aVar.b = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        y3aVar.c = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        y3aVar.d = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }
