...
public final class ByteQuadsCanonicalizer {
    ...
    private int _appendLongName(int[] iArr, int i) {
        ...
        if (i3 > iArr2.length) {
            this._hashArea = Arrays.copyOf(this._hashArea, Math.max(i3 - iArr2.length, Math.min(4096, this._hashSize)) + this._hashArea.length);
        }
    ...
    */
    ...
    private void _verifySharing() {
        if (this._hashShared) {
            if (this._parent == null) {
                if (this._count == 0) {
                    r6.g("Internal error: Cannot add names to Root symbol table");
                    return;
                } else {
                    r6.g("Internal error: Cannot add names to Placeholder symbol table");
                    return;
                }
            }
            int[] iArr = this._hashArea;
    ...
    private void rehash() throws StreamConstraintsException {
        ...
        r6.g(k20.l("Internal error: Failed rehash(), old count=", i2, i6, ", copyCount="));
    }
    ...
    public void _reportTooManyCollisions() throws StreamConstraintsException {
        ...
        throw new StreamConstraintsException(w20.k(sb, this._hashSize >> 3, " slots -- suspect a DoS attack based on hash collisions. You can disable the check via `JsonFactory.Feature.FAIL_ON_SYMBOL_HASH_OVERFLOW`"));
    }
    ...
    public int calcHash(int[] iArr, int i) {
        if (i < 4) {
            l5.q("qlen is too short, needs to be at least 4");
            return 0;
        }
