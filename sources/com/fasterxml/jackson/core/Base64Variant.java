...
public final class Base64Variant implements Serializable {
    ...
    public Base64Variant(String str, String str2, boolean z, char c, int i) {
        int[] iArr = new int[IntBufferBatchMountItem.INSTRUCTION_UPDATE_LAYOUT];
        ...
        if (length != 64) {
            l5.q(bg.d(length, "Base64Alphabet length must be exactly 64 (was ", ")"));
            throw null;
        }
    ...
    public void _reportInvalidBase64(char c, int i, String str) {
        String strM;
        if (c <= ' ') {
            strM = "Illegal white space character (code 0x" + Integer.toHexString(c) + ") as character #" + (i + 1) + " of 4-char base64 unit: can only used between units";
        } else if (usesPaddingChar(c)) {
            strM = "Unexpected padding character ('" + getPaddingChar() + "') as character #" + (i + 1) + " of 4-char base64 unit: padding only legal as 3rd or 4th character";
        } else if (!Character.isDefined(c) || Character.isISOControl(c)) {
            strM = "Illegal character (code 0x" + Integer.toHexString(c) + ") in base64 content";
        } else {
            strM = "Illegal character '" + c + "' (code 0x" + Integer.toHexString(c) + ") in base64 content";
        }
        if (str != null) {
            strM = z3.m(strM, ": ", str);
        }
    ...
    public String unexpectedPaddingMessage() {
        return l5.m("Unexpected end of base64-encoded String: base64 variant '", getName(), "' expects no padding at the end while decoding. This Base64Variant might have been incorrectly configured");
    }
