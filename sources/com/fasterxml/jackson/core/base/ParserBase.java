...
public abstract class ParserBase extends ParserMinimalBase {
    ...
    public static int[] growArrayBy(int[] iArr, int i) {
        ...
        l5.q("Unable to grow array to longer than `Integer.MAX_VALUE`");
        return null;
    }
    ...
    public BigDecimal _getBigDecimal() throws JsonParseException {
        ...
        if (str == null) {
            r6.g("cannot get BigDecimal from current parser state");
            return null;
        }
        try {
    ...
    public BigInteger _getBigInteger() throws JsonParseException {
        ...
        String str = this._numberString;
        if (str == null) {
            r6.g("cannot get BigInteger from current parser state");
            return null;
        }
        try {
    ...
    public IllegalArgumentException reportInvalidBase64Char(Base64Variant base64Variant, int i, int i2, String str) {
        String strM;
        if (i <= 32) {
            strM = String.format("Illegal white space character (code 0x%s) as character #%d of 4-char base64 unit: can only used between units", Integer.toHexString(i), Integer.valueOf(i2 + 1));
        } else if (base64Variant.usesPaddingChar(i)) {
            strM = "Unexpected padding character ('" + base64Variant.getPaddingChar() + "') as character #" + (i2 + 1) + " of 4-char base64 unit: padding only legal as 3rd or 4th character";
        } else if (!Character.isDefined(i) || Character.isISOControl(i)) {
            strM = "Illegal character (code 0x" + Integer.toHexString(i) + ") in base64 content";
        } else {
            strM = "Illegal character '" + ((char) i) + "' (code 0x" + Integer.toHexString(i) + ") in base64 content";
        }
        if (str != null) {
            strM = z3.m(strM, ": ", str);
        }
        return new IllegalArgumentException(strM);
    }
