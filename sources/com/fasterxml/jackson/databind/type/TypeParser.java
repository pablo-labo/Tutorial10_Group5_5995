...
public class TypeParser implements Serializable {
    ...
    private static String _quoteTruncated(String str) {
        return str.length() <= 1000 ? l5.m("'", str, "'") : String.format("'%s...'[truncated %d charaters]", str.substring(0, 1000), Integer.valueOf(str.length() - 1000));
    }

    public IllegalArgumentException _problem(MyTokenizer myTokenizer, String str) {
        StringBuilder sbF = u40.f("Failed to parse type ", _quoteTruncated(myTokenizer.getAllInput()), " (remaining: ", _quoteTruncated(myTokenizer.getRemainingInput()), "): ");
        sbF.append(str);
        return new IllegalArgumentException(sbF.toString());
    }
    ...
    public JavaType parse(String str) {
        if (str.length() > 64000) {
            t40.q("Failed to parse type %s: too long (%d characters), maximum length allowed: %d", new Object[]{_quoteTruncated(str), Integer.valueOf(str.length()), 64000});
            return null;
        }
