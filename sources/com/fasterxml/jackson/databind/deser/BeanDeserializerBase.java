...
public abstract class BeanDeserializerBase extends StdDeserializer<Object> implements ContextualDeserializer, ResolvableDeserializer, Serializable {
    ...
    public SettableBeanProperty _resolveManagedReferenceProperty(DeserializationContext deserializationContext, SettableBeanProperty settableBeanProperty) throws InvalidDefinitionException {
        ...
        if (settableBeanPropertyFindBackReference == null) {
            return (SettableBeanProperty) deserializationContext.reportBadDefinition(javaType, t40.l("Cannot handle managed/back reference ", ClassUtil.name(managedReferenceName), ": no back reference property found from type ", ClassUtil.getTypeDescription(settableBeanProperty.getType())));
        }
        ...
        if (!type.getRawClass().isAssignableFrom(javaType.getRawClass())) {
            ...
            StringBuilder sbF = u40.f("Cannot handle managed/back reference ", strName, ": back reference type (", typeDescription, ") not compatible with managed type (");
            sbF.append(name);
            sbF.append(")");
            deserializationContext.reportBadDefinition(javaType2, sbF.toString());
        }
        ...
        if (member != null && (objectIdInfoFindObjectIdInfo = annotationIntrospector.findObjectIdInfo(member)) != null) {
            ...
            if (generatorType == ObjectIdGenerators$PropertyGenerator.class) {
                ...
                if (settableBeanPropertyFindProperty == null) {
                    return (JsonDeserializer) deserializationContext.reportBadDefinition(this._beanType, t40.l("Invalid Object Id definition for ", ClassUtil.nameOf(handledType()), ": cannot find property with name ", ClassUtil.name(propertyName)));
                }
    ...
    public void resolve(DeserializationContext deserializationContext) throws InvalidDefinitionException {
        ...
        if (this._valueInstantiator.canCreateUsingDelegate()) {
            ...
            if (delegateType == null) {
                JavaType javaType = this._beanType;
                deserializationContext.reportBadDefinition(javaType, akb.k("Invalid delegate-creator definition for ", ClassUtil.getTypeDescription(javaType), ": value instantiator (", ClassUtil.classNameOf(this._valueInstantiator), ") returned true for 'canCreateUsingDelegate()', but null for 'getDelegateType()'"));
            }
        ...
        if (this._valueInstantiator.canCreateUsingArrayDelegate()) {
            ...
            if (arrayDelegateType == null) {
                JavaType javaType2 = this._beanType;
                deserializationContext.reportBadDefinition(javaType2, akb.k("Invalid delegate-creator definition for ", ClassUtil.getTypeDescription(javaType2), ": value instantiator (", ClassUtil.classNameOf(this._valueInstantiator), ") returned true for 'canCreateUsingArrayDelegate()', but null for 'getArrayDelegateType()'"));
            }
