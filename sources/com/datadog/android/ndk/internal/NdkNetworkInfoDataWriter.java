...
public final class NdkNetworkInfoDataWriter extends SingleItemDataWriter<NetworkInfo> {
    ...
    public NdkNetworkInfoDataWriter(File file, ConsentProvider consentProvider, ExecutorService executorService, FileWriter<byte[]> fileWriter, FileMover fileMover, InternalLogger internalLogger, FilePersistenceConfig filePersistenceConfig) {
        file.getClass();
        ...
        super(new ConsentAwareFileOrchestrator(consentProvider, new SingleFileOrchestrator(companion.getPendingNetworkInfoFile$dd_sdk_android_core_release(file), internalLogger), new SingleFileOrchestrator(companion.getGrantedNetworkInfoFile$dd_sdk_android_core_release(file), internalLogger), new ConsentAwareFileMigrator(fileMover, internalLogger), executorService, internalLogger), new NetworkInfoSerializer(), fileWriter, internalLogger, filePersistenceConfig);
    }
}
