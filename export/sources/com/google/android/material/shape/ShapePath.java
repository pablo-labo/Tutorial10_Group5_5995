...
public class ShapePath {
    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix matrix, ShadowRenderer shadowRenderer, int i, Canvas canvas) {
            ...
            double d = fMin;
            double dTan = Math.tan(Math.toRadians((-sweepAngle) / 2.0f));
            Double.isNaN(d);
            double d2 = dTan * d;
            if (dHypot > d2) {
                RectF rectF = new RectF(0.0f, 0.0f, (float) (dHypot - d2), 0.0f);
                this.renderMatrix.set(matrix);
                this.renderMatrix.preTranslate(this.startX, this.startY);
            ...
            Double.isNaN(d);
            matrix2.preTranslate((float) ((-d2) - d), (-2.0f) * fMin);
            Double.isNaN(d);
            shadowRenderer.drawInnerCornerShadow(canvas, this.renderMatrix, rectF2, (int) fMin, 450.0f, sweepAngle, new float[]{(float) (d + d2), f});
            if (dHypot2 > d2) {
                RectF rectF3 = new RectF(0.0f, 0.0f, (float) (dHypot2 - d2), 0.0f);
                this.renderMatrix.set(matrix);
                this.renderMatrix.preTranslate(this.operation1.x, this.operation1.y);
