    ...
    public static List<Integer> score(Map<Integer, Integer> map, int i, int i2, boolean z) {
        ...
        for (int i3 = 0; i3 < 360; i3++) {
            ...
            for (int i4 = i3 - 14; i4 < i3 + 16; i4++) {
                ...
                dArr[iSanitizeDegreesInt] = dArr[iSanitizeDegreesInt] + d4;
            }
        }
        ArrayList arrayList2 = new ArrayList();
        for (Hct hct : arrayList) {
            double d5 = dArr[MathUtils.sanitizeDegreesInt((int) Math.round(hct.getHue()))];
            if (!z || (hct.getChroma() >= CUTOFF_CHROMA && d5 > CUTOFF_EXCITED_PROPORTION)) {
                arrayList2.add(new ScoredHCT(hct, (d5 * 100.0d * WEIGHT_PROPORTION) + ((hct.getChroma() - TARGET_CHROMA) * (hct.getChroma() < TARGET_CHROMA ? WEIGHT_CHROMA_BELOW : WEIGHT_CHROMA_ABOVE))));
            }
        }
